...
from llmcore import reload_mykeys, ToolClient, MixinSession, NativeToolClient, NativeClaudeSession, NativeOAISession, \
    resolve_client
...
def get_system_prompt():
    with open(os.path.join(script_dir, f'assets/sys_prompt{lang_suffix}.txt'), 'r',
              encoding='utf-8') as f: prompt = f.read()
...
class GenericAgent:
    ...
    def run(self):
        while True:
            ...
            sys_prompt = get_system_prompt() + '\n'.join(self.extra_sys_prompts) + getattr(self.llmclient.backend,
                                                                                           'extra_sys_prompt', '')
            ...
            try:
                ...
                display_queue.put(
                    {'done': full_resp, 'source': source, 'turn': curr_turn, 'outputs': turn_resps.copy()})
    ...
    if args.task:
        ...
        while True:
            ...
            if task:
                ...
                open(os.path.join(log_dir, f'{script_name}_{datetime.now():%Y-%m-%d}.log'), 'a',
                     encoding='utf-8').write(f'[{datetime.now():%m-%d %H:%M}]\n{result}\n\n')
