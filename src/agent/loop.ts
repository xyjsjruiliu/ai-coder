...
export class AgentLoop {
   ...
   */
  async *run(userInput: string): AsyncGenerator<StreamChunk & { _meta?: { turn: number } }> {
    ...
    while (
      ...
      try {
          ...
          switch (chunk.type) {
            ...
            case 'stop':
              ...
              if (chunk.usage) {
                ...
                this.state.lastTurnTokens = { input: turnTokens.input, output: turnTokens.output };
              }
              break;
        ...
        log('error', `Agent turn ${this.state.turns} failed (${this.consecutiveErrors}/${this.config.maxConsecutiveErrors}): ${err.message}`);
      ...
      if (callsToExecute.length > 0) {
        ...
        if (this.config.debug) {
          const summary = toolResults.map((m) => {
